"""
InterviewLoop — PDF Export Module (Phase 5)
...
# ─── Color palette matching InterviewLoop dark theme ──────────────────
...
def generate_session_pdf(session_id: str, db) -> str:
    ...
    # Header
    story.append(Paragraph("🔄 InterviewLoop Interview Report", styles["Title2"]))
    ...
    story.append(Paragraph(
        "This report was generated by InterviewLoop and is cryptographically signed with RSA-2048. "
...
def generate_growth_pdf(candidate_id: str, db) -> str:
    ...
    # Header
    story.append(Paragraph("📈 InterviewLoop Growth Report", styles["Title2"]))
    story.append(Paragraph(
    ...
    story.append(HRFlowable(width="100%", thickness=0.5, color=BORDER))
    story.append(Paragraph(
        "InterviewLoop — AI-Powered Mock Interview Platform. Keep practicing — every session counts!",
        ParagraphStyle("Footer", parent=styles["Normal"], fontSize=8, textColor=MUTED, alignment=TA_CENTER),
    ))
    doc.build(story)
    return out_path
